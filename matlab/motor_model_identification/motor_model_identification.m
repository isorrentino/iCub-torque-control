...
reg = [mtr_curr_mA, -mtr_vel_deg_sec];
k = reg \ joint_trq;
...
hold on
scatter(0:length(joint_trq)-1, invKtau*mtr_curr_mA - kbemf*mtr_vel_deg_sec)
xlabel('time')
ylabel('joint torque')
...
% RMSE = sqrt(mean((joint_trq - invKtau*mtr_curr_mA - kbemf*mtr_vel_deg_sec).^2));
...
% Negative velocities
% reg_neg = [mtr_curr_mA, -sgn(-1), -dq];
% reg_neg = [mtr_curr_mA, -sgn(1), -dq];

idx_neg = find(mtr_vel_deg_sec < 0);
idx_pos = find(mtr_vel_deg_sec > 0);

reg_neg = [mtr_curr_mA(idx_neg), ones(size(mtr_vel_deg_sec(idx_neg))), -mtr_vel_deg_sec(idx_neg)];
reg_pos = [mtr_curr_mA(idx_pos), -ones(size(mtr_vel_deg_sec(idx_pos))), -mtr_vel_deg_sec(idx_pos)];

k_neg = reg_neg \ joint_trq(idx_neg);
k_pos = reg_pos \ joint_trq(idx_pos);

invKtau_neg = k_neg(1);
Ktau_neg = 1/invKtau_neg;
kc_neg = k_neg(2);
kv_neg = k_neg(3);

invKtau_pos = k_pos(1);
Ktau_pos = 1/invKtau_pos;
kc_pos = k_pos(2);
kv_pos = k_pos(3);
...
hold on
scatter(0:length(joint_trq(idx_neg))-1, invKtau_neg*mtr_curr_mA(idx_neg) - kc_neg*sign(mtr_vel_deg_sec(idx_neg)) - kv_neg*mtr_vel_deg_sec(idx_neg))
xlabel('time')
ylabel('joint torque')
legend('measured','estimated')
...
hold on
scatter(0:length(joint_trq(idx_pos))-1, invKtau_pos*mtr_curr_mA(idx_pos) - kc_pos*sign(mtr_vel_deg_sec(idx_pos)) - kv_pos*mtr_vel_deg_sec(idx_pos))
xlabel('time')
ylabel('joint torque')
legend('measured','estimated')
...
% RMSE = sqrt(mean(([joint_trq(idx_neg);joint_trq(idx_pos)] - [invKtau_neg*mtr_curr_mA(idx_neg) + kc_neg*sign(mtr_vel_deg_sec(idx_neg)) - kv_neg*mtr_vel_deg_sec(idx_neg); invKtau_pos*mtr_curr_mA(idx_pos) - kc_pos*sign(mtr_vel_deg_sec(idx_pos)) - kv_pos*mtr_vel_deg_sec(idx_pos)]).^2));
...
% reg_neg = mtr_curr_mA(idx_neg);
% reg_pos = mtr_curr_mA(idx_pos);
...
% Ktau_neg_2 = 1/invKtau_neg_2;
% kc_neg = k_neg(2);
% kv_neg = k_neg(3);

% invKtau_pos_2 = k_pos_2(1);
% Ktau_pos_2 = 1/invKtau_pos_2;
% kc_pos = k_pos(2);
% kv_pos = k_pos(3);
...
% scatter(0:length(joint_trq(idx_neg))-1,joint_trq(idx_neg))
% hold on
% scatter(0:length(joint_trq(idx_neg))-1, invKtau_neg_2*mtr_curr_mA(idx_neg) + parameters{joint}.kc_neg*sign(mtr_vel_deg_sec(idx_neg)) - parameters{joint}.kv_neg*mtr_vel_deg_sec(idx_neg))
% xlabel('time')
% ylabel('joint torque')
% legend('measured','estimated')
...
% scatter(0:length(joint_trq(idx_pos))-1,joint_trq(idx_pos))
% hold on
% scatter(0:length(joint_trq(idx_pos))-1, invKtau_pos_2*mtr_curr_mA(idx_pos) - parameters{joint}.kc_pos*sign(mtr_vel_deg_sec(idx_pos)) - parameters{joint}.kv_pos*mtr_vel_deg_sec(idx_pos))
% xlabel('time')
% ylabel('joint torque')
% legend('measured','estimated')
% title(strcat(Joint_state.joints{joint},' - positive vel'))

% RMSE_2 = sqrt(mean(([joint_trq(idx_neg);joint_trq(idx_pos)] - [invKtau_neg_2*mtr_curr(idx_neg) + parameters{joint}.kc_neg*sign(mtr_vel_deg_sec(idx_neg)) - parameters{joint}.kv_neg*mtr_vel_deg_sec(idx_neg);invKtau_pos_2*mtr_curr_mA(idx_pos) - parameters{joint}.kc_pos*sign(mtr_vel_deg_sec(idx_pos)) - parameters{joint}.kv_pos*mtr_vel_deg_sec(idx_pos)]).^2));
