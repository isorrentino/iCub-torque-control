...
dx_reconstructed = cumtrapz(ddx_kf(joint,:)) / fs;
x_reconstructed = cumtrapz(dx_reconstructed) / fs + joint_pos(joint,1);

figure,
plot(joint_pos(joint,:))
hold on
plot(x_reconstructed)
xlabel('samples')
ylabel('joint position')
legend('measured position','reconstructed from ddq_kf (integrating twice)')
